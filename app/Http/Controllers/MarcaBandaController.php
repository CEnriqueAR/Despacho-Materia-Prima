...
class MarcaBandaController extends Controller
{
    //Funcion Lista Marca
    ...
    public function storeMarca(createMarcasRequest $request){
        ...
        $marca->save();
        return redirect()->route("marcasBanda")->withExito("Marca creada correctamente");
    }
    ...
    public function editarMarca(Request $request){
        ...
        if ($name!=null){
            try {
                ...
                $editar->save();
                return redirect()->route("marcasBanda")->withExito("Marca editada correctamente");
            }catch (ValidationException $exception){
                return redirect()->route("marcasBanda")->with('errores','errores')->with('id_M',$request->input("id"))->withErrors($exception->errors());
            }
        }else{
            try {
                $this->validate($request, [
                ...
                return redirect()->route("marcasBanda")->with('errors','errors')->with('id_M',$request->input("id"))->withErrors($exception->errors());
            }
        }
    ...
    public function borrarMarca(Request $request){
        ...
        $borrar->delete();
        return redirect()->route("marcasBanda")->withExito("Marca borrada con éxito");
    }
